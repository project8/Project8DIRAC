...
PATH_TO_SANDBOX = ('/project8/dirac/calib/esr/scripts')
...
class P8Transformation(Transformation):
    ...
    def buildTransformation(self):
        ...
        cwd = os.getcwd()
        # Create esr plot script 
        script = (
                '#!/bin/bash\n'
                'python -c "import p8dirac_esrplot_tools as tools; print(tools.getPlotJobLFNs())"\n'
                '#ls -l\n'
                '(source /cvmfs/hep.pnnl.gov/project8/katydid/v2.16.0/setup.sh; '
                'python -c "import p8_esrplot_tools as tools; print(tools.execute())")\n'
                'ls -l\n'
                'python -c "import p8dirac_esrplot_tools as tools; print(tools.uploadJobOutputROOT())"\n') 
        script_name = os.path.join(cwd, 'plot_esr.sh')
        f = open(script_name, 'w+')
        f.write(script)
        f.close()
        # Upload Plot script
        plot_file = os.path.join(
                path_to_sandbox,'plot_esr.sh')
        print('plot_file: %s\n' % plot_file)
        print('plot contents:\n%s\n' % script)
        ...
        # Upload tools file
        tools_file = os.path.join(path_to_sandbox, 'p8dirac_esrplot_tools.py')
        print('tools_file: %s\n' % tools_file)
        ...
        res = dirac.addFile(
                tools_file,
                os.path.join(cwd, 'p8dirac_esrplot_tools.py'),
                PROD_DEST_DATA_SE)
        #print(res)
        ...
        # Upload plots file
        plot_tools_file = os.path.join(path_to_sandbox, 'p8_esrplot_tools.py')
        ...
        res = dirac.addFile(
	        plot_tools_file,
	        os.path.join(cwd, 'p8_esrplot_tools.py'),
	        PROD_DEST_DATA_SE)
        #print(res)
        if not res['OK']:
            print('Could not add tools file.')
	...
	# Add upload file script
        upload_file = os.path.join(
                path_to_sandbox,'upload_esr.py')
        print('upload_file: %s\n' % upload_file)
        #print('plot contents:\n%s\n' % script)
        res = dirac.removeFile(upload_file) # First remove it
        if not res['OK']:
            return res
        res = dirac.addFile(upload_file, os.path.join(cwd, 'upload_esr.py'), PROD_DEST_DATA_SE)
        # Now check if the file was uploaded successfully
        if not res['OK']:
          return res
        # Remove the tools file regardless of if the file was uploaded successfully
        ...
        self.setDescription(
                'Transformation used to automatically plot calib esr data')
        self.setLongDescription(
                'Transformation use to automatically plot calib esr data. '
        ...
        inputDataQuery = {'DataType': 'calib', 'DataLevel': 'raw', 'DataFlavor': 'esr'}
        #Test 
