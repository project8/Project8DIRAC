...
__RCSID__ = "95edd25 (2016-12-01 18:36:17 +0100) Federico Stagni <federico.stagni@cern.ch>"
...
class ComponentInstaller( object ):
  ...
  """ % {'bashrc': os.path.join( self.instancePath, 'bashrc' ),
      ...
      if cTypeLower == 'agent' or cTypeLower == 'consumer':
        stopFile = os.path.join( runitCompDir, 'control', 't' ) # This is, e.g., /srv/dirac/runit/WorkfloadManagementSystem/Matcher/control/t
        controlDir = self.runitDir.replace('runit', 'control') # This is, e.g., /srv/dirac/control/WorkfloadManagementSystem/Matcher/
        ...
        for line in myOrg:
          if line.find( '[mysqld]' ) == 0:
            ...
            line = '\n'.join( ['server-id = %s' % int( time.time() ),
                               ...
                               'log-bin = mysql-bin',
                               'log-slave-updates', '' ] )
          elif line.find( '/srv/dirac/mysql' ) > -1:
            line = line.replace( '/srv/dirac/mysql', self.mysqlDir )
