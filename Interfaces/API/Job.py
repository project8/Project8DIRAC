...
__RCSID__ = "be33468 (2017-01-16 15:14:09 +0100) Federico Stagni <federico.stagni@cern.ch>"
...
class Job( API ):
  ...
  def __init__( self, script = None, stdout = 'std.out', stderr = 'std.err' ):
    ...
    self.stderr = stderr
    self.logLevel = 'info'
    #self.useSpecifiedOutputPath = 0 # VB: This is to enforce usgae of given outputpath without prepending VO/user to Outputpath
  ...
  # VB
  def setUseSpecifiedOutputPath( self, useSpecifiedOutputPath ):
    """Helper function.
        
        useSpecifiedOutputPath is only effective if Outputpath is also specified.
        useSpecifiedOutputPath : 1 means use given OutputPath, and 0 is ignore it (default)
        
        useSpecifiedOutputPath for the job can be specified if desired. This may NOT appear
        in the useSpecifiedOutputPath field of the monitoring webpage; if nothing is
        specified a default value will appear.
          
        Example usage:
          
        >>> job=Job()
        >>> job.setUseSpecifiedOutputPath(1)
          
        :param useSpecifiedOutputPath: use Specified OutputPath
        :type useSpecifiedOutputPath: int
    """
    kwargs = {'useSpecifiedOutputPath':useSpecifiedOutputPath}
    if not isinstance( useSpecifiedOutputPath, int ):
      try:
        useSpecifiedOutputPath = int( useSpecifiedOutputPath )
      except ValueError:
        if not re.search( '{{', useSpecifiedOutputPath ):
          return self._reportError( 'Expected numerical string or int for useSpecifiedOutputPath', **kwargs )
                
    description = 'use Specified OutputPath'
    self._addParameter( self.workflow, 'useSpecifiedOutputPath', 'JDL', useSpecifiedOutputPath, description )
    return S_OK()
