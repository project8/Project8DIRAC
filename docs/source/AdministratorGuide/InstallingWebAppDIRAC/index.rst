...
Before you start the installation please make sure that you have the host certificate in the /srv/dirac/etc directory... More info in the Server Certificates section in :ref:`server_requirements` .
...
Make sure that the portal is listening in the correct port::
   ...
   tail -200f /srv/dirac/runit/Web/WebApp/log/current
...
If you are not using NGinx and the web server is listening on 8000, please open vim /srv/dirac/pro/WebAppDIRAC/WebApp/web.cfg and add Balancer=None.
Make sure that the configuration /srv/dirac/pro/etc/dirac.cfg file is correct. It contains Extensions = WebApp. For example::
...
We use **web.cfg** configuration file. The location of the file is /srv/dirac/pro/WebAppDIRAC/WebApp/web.cfg The structure of the web.cfg file is the following::
...
* Configure NGINX
 ...
 The content of the site.conf (please modify it!!!)::
   ...
   server {
     ...
     ssl_certificate    /srv/dirac/etc/grid-security/hostcert.pem;
     ssl_certificate_key /srv/dirac/etc/grid-security/hostkey.pem;

     ssl_client_certificate /srv/dirac/pro/etc/grid-security/cas.pem;
   #  ssl_crl /srv/dirac/pro/etc/grid-security/allRevokedCerts.pem;
